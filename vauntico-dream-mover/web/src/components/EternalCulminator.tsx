...
export function EternalCulminator() {
  ...
  return (
    <div style={{ border: '1px solid #555', padding: 12, borderRadius: 8 }}>
      ...
      <button className="hover:scale-[1.02] hover:shadow-vauntico-glow transition-all duration-300" style={{ marginTop: 8 }} onClick={() => {
        const cmd = `node vauntico-dream-mover/dist/cli.js eternal-culminate --blitz ${blitz} --viral ${viral} --legacy ${legacy}`
        alert(`Run via Warp:\n${cmd}`)
      }}>Culminate Eternal?</button>
      <div style={{ marginTop: 6, fontSize: 12, opacity: 0.8 }}>Appends chain to eternal-launch.yml and writes logs/eternal-culminate.yml.</div>
    </div>
  )
}