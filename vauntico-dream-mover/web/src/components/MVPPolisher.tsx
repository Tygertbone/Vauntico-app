  ...
  return (
    <div style={{ border: '1px solid #555', padding: 12, borderRadius: 8 }}>
      ...
      <button className="hover:scale-[1.02] hover:shadow-vauntico-glow transition-all duration-300" style={{ marginTop: 8 }} onClick={() => {
        const cmd = `node vauntico-dream-mover/dist/cli.js polish-mvp --config ${config}`
        alert(`Run via Warp:\n${cmd}`)
      }}>Polish Eternal?</button>
      <div style={{ marginTop: 6, fontSize: 12, opacity: 0.8 }}>Writes logs/polish-mvp.json with DM_POLISH=true and full i18n dict (en/af).</div>
    </div>
  )
}